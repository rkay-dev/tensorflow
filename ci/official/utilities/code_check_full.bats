...
@test "Verify that it's possible to query every TensorFlow target without BUILD errors" {
    bazel query "deps(//tensorflow/... -//tensorflow/tools/pip_package:test_import_api_packages_cpu -//tensorflow/tools/pip_package:test_import_api_packages_gpu)" > /dev/null
}
